...
DEG_wordsize <- list(0.8, 1, 0.7)
DEG_word_width <- list(1.1e3, 1.05e3, 1.05e3)
DEG_word_height <- list(0.7e3, 0.65e3, 0.7e3)

GKPI_wordsize <- list(0.20, 0.80, 0.20, 0.20)
GKPI_word_width <- list(1.15e3, 1.35e3, 1.25e3, 1.1e3)
GKPI_word_height <- list(0.7e3, 0.65e3, 0.65e3, 0.7e3)

# Set size for gene names on heatmap (full DEGs list), and sample names
heatmap_row_wordsize <- 0.4
...
## Volcano plot ####
# GSK vs SGC
Volcano(data = subset(result, select = c(LFC_col, qval_col, Genes_col)), y_offset = 2.0,
        PlotTitle = paste(paste("Volcano", dataType, sep = " : "), cell, paste(condition1, "vs", condition2), sep = " : "), 
...
png(paste(currentStorage, "Fig", "Heatmap", paste("Heatmap", dataType, contrast, "euclidean_full.png", sep = "_"), sep = "/"),
    width = 15, height = 15, units = "in", res = 6e2)
...
# 
# ## Interaction effects ####
# table(allResult[[5]][["signif"]]);table(allResult[[6]][["signif"]]);table(allResult[[7]][["signif"]])
# LOAD if already have the files ####
GKPI_files <- list()
for (i in 1:length(GKPI_folders)) {
